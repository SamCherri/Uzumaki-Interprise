...
## Fase 2 (fluxo econômico central do MVP)

Nesta fase, o backend implementa o fluxo econômico fictício:

`ADM Chefe da Moeda -> Tesouraria Central -> Corretor Virtual -> Usuário`.

Reforço legal: tudo no sistema é **ficção de jogo/simulação econômica**, sem dinheiro real, sem saque real, sem criptoativo real e sem investimento real.

### Endpoints econômicos

- `POST /api/economy/treasury/issue`
  - cria moeda virtual na Tesouraria Central;
  - apenas `COIN_CHIEF_ADMIN`.
- `GET /api/economy/treasury`
  - consulta saldo da Tesouraria Central;
  - `COIN_CHIEF_ADMIN`, `ADMIN` ou `SUPER_ADMIN`.
- `POST /api/economy/treasury/transfer-broker`
  - envia moeda da Tesouraria para corretor virtual;
  - `COIN_CHIEF_ADMIN`, `ADMIN` ou `SUPER_ADMIN`.
- `GET /api/economy/broker/balance`
  - corretor virtual consulta saldo disponível e histórico básico.
- `POST /api/economy/broker/transfer-user`
  - corretor virtual repassa moeda para usuário.

### Regras de segurança econômica implementadas

- não permite saldo negativo na tesouraria;
- não permite corretor repassar mais do que possui;
- criação de moeda gera `CoinIssuance`;
- transferências geram `CoinTransfer`;
- repasse para usuário gera `Transaction` na carteira do destinatário;
- ações sensíveis geram `AdminLog`.
