...
import { CompanyStatus, MarketOrderMode, MarketOrderStatus, MarketOrderType, TransferType, WithdrawalStatus } from '@prisma/client';
import { z } from 'zod';
...
export async function adminAuditRoutes(app: FastifyInstance) {
  ...
  app.get('/audit/transfers', { preHandler: [app.authenticate] }, async (request, reply) => {
    if (!checkAdmin(reply, request)) return;
    const query = z.object({ type: z.nativeEnum(TransferType).optional(), senderId: z.string().optional(), receiverId: z.string().optional(), search: z.string().optional(), from: z.string().optional(), to: z.string().optional() }).merge(paginationSchema).parse(request.query);
  ...
  app.get('/audit/withdrawals', { preHandler: [app.authenticate] }, async (request, reply) => {
    if (!checkAdmin(reply, request)) return;
    const query = z.object({ status: z.nativeEnum(WithdrawalStatus).optional(), userId: z.string().optional(), code: z.string().optional(), search: z.string().optional(), from: z.string().optional(), to: z.string().optional() }).merge(paginationSchema).parse(request.query);
  ...
  app.get('/audit/orders', { preHandler: [app.authenticate] }, async (request, reply) => { /* similar */
    if (!checkAdmin(reply, request)) return;
    const query = z.object({ status: z.nativeEnum(MarketOrderStatus).optional(), type: z.nativeEnum(MarketOrderType).optional(), mode: z.nativeEnum(MarketOrderMode).optional(), companyId: z.string().optional(), userId: z.string().optional(), search: z.string().optional(), from: z.string().optional(), to: z.string().optional() }).merge(paginationSchema).parse(request.query);
  ...
  app.get('/reports/overview', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const [users, brokers, active, suspended, closed, wallets, treasury, platform, pendingWithdrawals, openOrders, traded, companyFees] = await Promise.all([
      prisma.user.count(), prisma.user.count({ where: { roles: { some: { role: { key: 'VIRTUAL_BROKER' } } } } }), prisma.company.count({ where: { status: CompanyStatus.ACTIVE } }), prisma.company.count({ where: { status: CompanyStatus.SUSPENDED } }), prisma.company.count({ where: { status: CompanyStatus.CLOSED } }), prisma.wallet.aggregate({ _sum: { availableBalance: true, lockedBalance: true, pendingWithdrawalBalance: true } }), prisma.treasuryAccount.findFirst(), prisma.platformAccount.findFirst(), prisma.withdrawalRequest.count({ where: { status: WithdrawalStatus.PENDING } }), prisma.marketOrder.count({ where: { status: { in: [MarketOrderStatus.OPEN, MarketOrderStatus.PARTIALLY_FILLED] } } }), prisma.trade.aggregate({ _sum: { grossAmount: true } }), prisma.companyRevenueAccount.aggregate({ _sum: { totalReceivedFees: true } }),
    ]);
