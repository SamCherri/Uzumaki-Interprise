...
test('modo teste global bloqueia rotas reais, mantém isolamento e registra logs administrativos', async () => {
  ...
  const blockedMe = await app.inject({ method: 'GET', url: '/api/me', headers: { authorization: `Bearer ${userTk}` } });
  assert.equal(blockedMe.statusCode, 403);
  const blockedNoTokenMe = await app.inject({ method: 'GET', url: '/api/me' });
  assert.equal(blockedNoTokenMe.statusCode, 403);
