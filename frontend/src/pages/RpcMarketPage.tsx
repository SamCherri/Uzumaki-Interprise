  ...
  return (
    <section className="card market-page market-shell market-page-v2">
      <div className="trade-screen market-mobile-shell">
        ...
        {activeTab === 'livro' && <section className="card nested-card market-tab-panel market-full-width"><h4>Livro de ordens RPC/R$</h4><p className="info-text">RPC/R$ ainda usa liquidez automática para execução. O livro mostra ordens limite pendentes.</p><div className="order-book-grid"><div className="summary-item"><span className="summary-label">Reserva RPC</span><strong>{formatCurrency(Number(market?.rpcReserve ?? 0))} RPC</strong></div><div className="summary-item"><span className="summary-label">Reserva R$</span><strong>R$ {formatCurrency(Number(market?.fiatReserve ?? 0))}</strong></div></div><OrderBook buyOrders={orderBook.buyOrders.map((o) => ({ id: o.id, price: Number(o.limitPrice), quantity: Number(o.lockedFiatAmount || o.fiatAmount || 0), total: Number(o.lockedFiatAmount || o.fiatAmount || 0) }))} sellOrders={orderBook.sellOrders.map((o) => { const price = Number(o.limitPrice || 0); const quantity = Number(o.lockedRpcAmount || o.rpcAmount || 0); return { id: o.id, price, quantity, total: price * quantity }; })} currentPrice={Number(market?.currentPrice ?? 0)} quoteSymbol="R$" baseSymbol="RPC" emptyMessage="Sem ordens abertas neste livro." /></section>}
